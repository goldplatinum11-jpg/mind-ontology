...
const EXPECTED_FENCES = [
  { section: "The inversion: static files as targets, not sources", lang: "text", class: "illustrative", marker: ".agentctx/ source files" },
  { section: "Try it in 30 seconds", lang: "sh", class: "command-example", marker: "npm install" },
  { section: "Try it in 30 seconds", lang: "text", class: "verified-output", marker: "WROTE  AGENTS.md" },
  { section: "Try it in 30 seconds", lang: "text", class: "verified-output", marker: "<!-- mind-ontology:emit" },
  { section: "Try it in 30 seconds", lang: "sh", class: "command-example", marker: "emit --check" },
  { section: "Try it in 30 seconds", lang: "text", class: "verified-output", marker: "STALE" },
  { section: "Try it in 30 seconds", lang: "text", class: "verified-output", marker: "OK - 2 of 2 targets fresh" },
  { section: "Drift fails CI", lang: "sh", class: "command-example", marker: "npx mind-ontology emit --check" },
  { section: "The live path: compile per task, not per file", lang: "sh", class: "command-example", marker: "compile --task" },
  { section: "The live path: compile per task, not per file", lang: "json", class: "illustrative", marker: ".mcp.json" },
  { section: "The live path: compile per task, not per file", lang: "text", class: "illustrative", marker: "get_context(task)" },
  { section: "The live path: compile per task, not per file", lang: "sh", class: "command-example", marker: "setup --target claude-code" },
  { section: "The live path: compile per task, not per file", lang: "sh", class: "command-example", marker: "agentctx:proof" },
  { section: "The live path: compile per task, not per file", lang: "sh", class: "command-example", marker: "--risk auto" },
];
  ...
  it("info strings are deterministic per class: sh commands, text/json otherwise", () => {
    for (const entry of EXPECTED_FENCES) {
      if (entry.class === "command-example") {
        ...
        expect(["text", "json"], `${entry.class} "${entry.marker}" has executable-looking info string`).toContain(entry.lang);
      }
    }
