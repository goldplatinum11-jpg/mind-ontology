...
const SETUP_PROOF = "tests/unit/agent-setup-command.test.mjs";
...
const EXPECTED_FENCES = {
  "docs/agent-setup.md": [
    { section: null, lang: "sh", class: "command-example", marker: "mind-ontology agent-setup --target codex --cwd <project> --print" },
...
describe("setup docs illustrative fences stay inert / delegated", () => {
  ...
  it("the bootstrap instruction fence names the two-tool contract and is pinned by the setup proof", () => {
    ...
    // must be re-judged.
    expect(docText["docs/agent-setup.md"], `doc no longer cites ${SETUP_PROOF}`).toContain("agent-setup-command.test.mjs");
