...
| Failure | Message (stderr) | Next safe action |
|---|---|---|
| Unknown target id | `--target must be one of "agents-md", "claude-md", "cursor", "paste-block", got: <x>` | Use a supported registry id (`agents-md`, `claude-md`, `cursor`, `paste-block`; `cursor` / `paste-block` are supported-but-not-default). |
