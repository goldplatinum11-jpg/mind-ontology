# Agent setup — adoption autoload (`mind-ontology setup`)

Mind Ontology only helps an AI coding agent if the agent actually reads it.
`mind-ontology setup` makes that wiring a one-command step instead of a
...
```sh
mind-ontology setup --target claude-code --cwd <project> --print
mind-ontology setup --target codex --cwd <project> --print
```
...
is a non-generated instruction layer, and `setup` does not write to files it
does not own.
...
`setup` works in a project that has not run `mind-ontology init` yet — it
...
```sh
mind-ontology setup --target claude-code --print          # inspect the plan
mind-ontology setup --target claude-code                  # write .mcp.json
...
The command's behavior is locked by `tests/unit/setup-command.test.mjs`.
